...
class Node2D {
 public:
  ...
  /// set the x position
  void setX(const int& x) { this->x = x; }//单位是grid
  /// set the y position
  void setY(const int& y) { this->y = y; }//单位是grid
  ...
  // 行走代价，定义为欧式距离
  float movementCost(const Node2D& pred) const { return sqrt((x - pred.x) * (x - pred.x) + (y - pred.y) * (y - pred.y)); }//单位是grid
